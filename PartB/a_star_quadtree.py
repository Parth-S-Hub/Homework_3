...
def a_star_quadtree(start_m, goal_m, qtm, movement='8n', occupancy_cost_factor=3):
    ...
    start_node_cost = 0
    initial_cost_estimate =  quadtreemap.Point.disOf2Points(start.getCenter(),goal.getCenter()) + start_node_cost

    candidates.put((initial_cost_estimate, start_node_cost, None, start))   # store (cost, distance, previous-tile, current-tile)
    while candidates:
        cost, dis, prev_node, curr_node = candidates.get()
